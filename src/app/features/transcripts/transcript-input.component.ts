...
@Component({
  ...
  template: `
    <section class="input-card">
      ...
      @if (!hasApiKey()) {
        <div class="warning" role="alert">Set your Anthropic API key in Settings before generating.</div>
      }
...
export class TranscriptInputComponent {
  ...
  readonly canGenerate = computed(
    () => this.hasApiKey() && !this.streaming() && this.hasAtLeastOneTranscript(),
  );
  ...
  async onGenerate(): Promise<void> {
    ...
    let accumulated = '';
    try {
      const awsOptions = isAwsApiKey(apiKey)
        ? { workspaceId: this.settings.awsWorkspaceId(), region: this.settings.awsRegion() }
        : undefined;
      const extras = {
        enableWebSearch: this.webSearchActive(),
        mcpServers: resolveEnabledMcps(this.packs.activePack().enabledMcps),
      };

      for await (const chunk of this.anthropic.streamTranscriptScript(
        sources,
        apiKey,
        awsOptions,
        this.selectedModel(),
